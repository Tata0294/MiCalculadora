...
public class Metodos {
    ...
    /**** Aqui se obtiene el operador matematico para nuestra operacion *****/
    // es un metodo estatico con visivilidad publica que retorna un string
    //recibe un parametro de tipo strinf que va hacer la operacion
    ...
    /***** Aqui se resuelve la cuenta matematica *****/
    public static void tryResolve(boolean fromResult, EditText etInput, OnResolveCallback callback) {
        //Se extrae la operación que se contiene en el editText suministrado
        String operation = etInput.getText().toString();

        //si la operacion esta vacia, retornamos y no hacemos absolutamente nada
        if (operation.isEmpty()) {
            return;
        }

        //Miramos si la operacion es viable
        if (operation.contains(Constantes.POINT) &&
                operation.lastIndexOf(Constantes.POINT) == operation.length() - 1) {
            operation = operation.substring(0, operation.length() - 1);
        }

        /*****Usa el metodo de obtener el tipo de operador *****/
        String operator = Metodos.getOperator(operation);
        String[] values = new String[0];

        //preguntamos si el opreador es diferente de null
    ...
    /****** Si las operaciones son optimas retorna una operacion matematica *****/
